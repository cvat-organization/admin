  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
        "http://localhost:4000/admin/dashboard/get-users-vs-activity",
        {
          headers: {
      ...
      if (selectedRange === "custom" && startDate && endDate) {
        updateDisplayDataForCustomRange(ActivityData, startDate, endDate);
      } else {
        updateDisplayData(ActivityData, selectedRange);
      }
    } catch (error) {
