...
import { userData } from "../data/d"; // Import JSON data
...
const UserChart: React.FC<Props> = ({ selectedRange }) => {
  ...
  const updateDisplayData = (range: string): void => {
    switch (range) {
      case "7days":
        setDisplayData(userData.slice(-7)); // Display last 7 days directly from userData
        break;
      case "1month":
        setDisplayData(userData.slice(-30)); // Display last 30 days (approx. 1 month) directly from userData
        break;
      case "6months":
